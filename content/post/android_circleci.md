...
draft: false
keywords: ["android","ci","cd","continuous-integration","continious-deployment","circleci"]
description: "Android Circle CI/CD"
tags: ["android","ci","cd","continuous-integration","continious-deployment","circleci"]
categories: []
author: "Aykut Asil"
comment: true
toc: true
...
Uygulamaların build/test/depleyment süreçlerini otomatize etmek anlamına gelen CI/CD kavramları günümüz yazılım dünyasında olmazsa olmaz, bilinmezse olmazlar arasına girmiş bulunmaktadır. Manuel yapılan bu işlemlerin otomatize edilmesinin fayda kat sayısı takım sayısına bağlı gibi gözükse de bir süre sonra durum ne olursa olsun süreç çeşitli problemlere gebe olmaktadır ve çözüm otomatize etmekten geçer. Bu sorunları yaşamaya başlamışsanız ya da başlamadan bitirmek istiyorsanız bu kavramları öğrenmekte fayda var.

Çeşitli CI/CD araçları bulunmaktadır. **Travis, Bitrise, AppCenter, CircleCI** bunlardan bazılarıdır.

> **Continuous Integration (CI)**

> Sürekli entegrasyon, geliştiricilerin kodlarını erken ve sık olarak paylaşılan bir reponun master dalına entegre etmelerini teşvik eden bir uygulamadır. Geliştirme yapılan kod sürecin en sonunda entegre edilmektense, gün içerisinde sık sık entegre edilerek testlerin çalıştırılması ve eğer varsa hataların önceden fark edilerek düzeltilmesi sağlanır. Sürekli Entegrasyon, dijital dönüşüm için önemli bir adımdır.
...
Circle CI open source projelere ücretsiz CI/CD araçları sunan bir ortamdır. Hazırlanan konfigurasyon dosyası(.yml) ile tüm süreçler otomatik olarak işletilir. Şimdi CircleCI ortamanın **Android** için sunduğu olanakları ve araçları hep beraber inceleyelim.
...
**steps** bloğu genelde projede bir job oluşturmak için gereken adımları/komutları belirtmemizi sağlar. **checkout** özel bir konuttur ve SSH üzerinde kaynak kodunu kontrol eder.**run** bölümünde ise çalıştırılacak komutlar belirtilir.

---

## save_cache

Uygulama bağımlılıklarını/dependency cache'lemek için kullanılır.
...
- Gerçek zamanlı durum geri bildirim ile işleri bağımsız olarak çalıştırma ve sorunları giderme,
...
**Jobs**, **steps** koleksiyonudur.
Jobs içerisindeki tüm steps'ler, çalışırken bir CircleCI konteynerini kullanan tek bir ünitede gerçekleştirilir.
